...
(* Only for well founded relations *)
Definition tst: Acc (fun x y => (y = x + 1)) 1.
Proof.
  constructor.
  intros y ey.
  enough (y = 0).
...
Goal wf (fun x y => (y = x + 1)).
Proof.
intros x.
induction x.
- constructor. intros y hy. exfalso. lia.
...
(* 26.1.2 *)
Fact ex2612: forall X R (x: X), Acc R x <-> (forall y, R y x -> Acc R y).
Proof.
  intros X R x.
  split.
  - intros [H] y Ryx.
...
(* 26.1.3 *)
Fact ex2613: forall X R (x: X), Acc R x -> ~ R x x.
Proof.
  intros X R x.
  Print W'.
  apply (W' (p := fun x => ~R x x)).
  intros x' H' H''.
  apply (H' x' H'').
  apply H''.
Qed.
...
(* 26.1.4 *)
Fact ex2614: forall X (R: X -> X -> Prop) (x y: X), 
  R x y -> R y x -> ~ Acc R x.
Proof.
  refine (fix F X R x y H1 H2 H {struct H} := let (f) := H in _).
  apply (F X R y x).
  - apply H2.
  - apply H1.
  - apply f.
