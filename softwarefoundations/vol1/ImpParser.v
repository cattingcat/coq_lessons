...
From LF Require Import Maps Imp2.
...
Example tokenize_ex1 :
  ...
  = ["abc"; "12"; "="; "3"; "223";
       "*"; "("; "3"; "+"; "(";
       "a"; "+"; "c"; ")"; ")"]%string.
Proof. unfold tokenize. simpl. reflexivity. Qed.
...
Notation "'TRY' ' p <- e1 ;; e2 'OR' e3"
   ...
   (right associativity, p pattern, at level 60, e1 at next level, e2 at next level).
...
Definition many {T} (p : parser T) (steps : nat) : parser (list T) := many_helper p [] steps.
...
Definition expect (t : token) : parser unit := firstExpect t (fun xs => SomeE (tt, xs)).
...
Fixpoint parsePrimaryExp (steps:nat) (xs : list token): optionE (aexp * list token) :=
  match steps with
...
Fixpoint parseAtomicExp (steps:nat) (xs : list token) : optionE (bexp * list token) :=
  match steps with
  | 0         => NoneE "Too many recursive calls"
  | S steps'  =>
...
Fixpoint parseSimpleCommand (steps:nat) (xs : list token) : optionE (com * list token) :=
  match steps with
  | 0         => NoneE "Too many recursive calls"
  | S steps'  =>
